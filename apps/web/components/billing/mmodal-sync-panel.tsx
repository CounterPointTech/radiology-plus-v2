      ...
      {!configured ? (
        <div className="rounded-lg border border-[color:var(--color-accent)]/40 bg-[color:var(--color-accent)]/10 px-4 py-3">
          <div className="flex items-center gap-2 text-sm font-medium">
            ...
            No M*Modal connection yet.
          </div>
          <p className="text-sm text-[color:var(--color-muted-fg)] mt-1 ml-6">
            Connect this site to its M*Modal database first — go to{" "}
            <Link
              href="/admin/mmodal-connection"
              className="text-[color:var(--color-accent)] underline underline-offset-2"
            >
              Admin → M*Modal connection
            </Link>
            . Nothing is written until a connection is set up.
          </p>
        </div>
      ) : (
        <>
